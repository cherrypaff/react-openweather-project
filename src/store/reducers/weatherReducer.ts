...
import { CityWeatherStatsType, ISOdate } from "network/types"
import { fetchWeather, fetchMyLocationWeather } from "store/fetchWeather"
    ...
    builder
      .addCase(fetchWeather.fulfilled, (state, action) => {
        if(action.payload) {
          ...
          const transFormedData: CityWeatherType = {
            graphData: [],
            metaData: {
              isPositive: true,
              isFahrenheit: true,
          ...
          action.payload.list.forEach((el: CityWeatherStatsType, i: number) => {
            if (i === 0) {
      ...
      .addCase(fetchWeather.rejected, (state, action) => {
        console.log("fetchWeather.rejected", action.payload)
      })
      .addCase(fetchMyLocationWeather.rejected, (state, action) => {
        console.log("fetchMyLocationWeather.rejected", action.payload)
      })
  },
})
